...
impl<'tree> GoExtractor<'_, 'tree> {
    ...
    fn go_vis(name: &str) -> Option<synaptic_core::Visibility> {
        match name.chars().next() {
            Some(c) if c.is_uppercase() => Some(synaptic_core::Visibility::Public),
            Some(_) => Some(synaptic_core::Visibility::Private),
            None => None,
        }
    }
    ...
    fn walk_type_spec(&mut self, spec: TsNode<'tree>) {
        ...
        let kind = Self::children(spec)
            ...
            .find_map(|c| match c.kind() {
                "struct_type" => Some(synaptic_core::NodeKind::Struct),
                "interface_type" => Some(synaptic_core::NodeKind::Interface),
                _ => None,
            })
            .unwrap_or(synaptic_core::NodeKind::TypeAlias);
...
mod tests {
    ...
    fn cgo_calls_emit_binds_native_edges() {
        ...
        assert_eq!(edge.confidence, synaptic_core::Confidence::Inferred);
