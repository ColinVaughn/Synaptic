...
use codegraph_core::{FileType, NodeId};
use codegraph_graph::{find_import_cycles, graph_diff, KnowledgeGraph};
    ...
    let mut out: Vec<RemovedApi> = old
        ...
        .filter_map(|n| {
            ...
            let is_api = match n.visibility() {
                Some(codegraph_core::Visibility::Public) => true,
...
mod tests {
    use super::*;
    use codegraph_core::{Confidence, Edge, GraphData, Node};
    use serde_json::Map;
    ...
    fn removed_apis_uses_visibility_when_present() {
        ...
        pubsym.set_visibility(codegraph_core::Visibility::Public);
        let mut privsym = n("priv_", "lib.rs");
        privsym.set_visibility(codegraph_core::Visibility::Private);
        let old = kg(
