//! Criterion benchmarks for `codegraph-graph` algorithm kernels.
//!
//! Two input modes:
...
//!
//! Run: `cargo bench -p codegraph-graph`
...
use codegraph_core::{Confidence, Edge, FileType, ImportRecord, Node, NodeId, RawCall};
use codegraph_graph::{
...
fn real_fixture() -> &'static RealFixture {
    ...
    FIXTURE.get_or_init(|| {
        ...
        for path in &files {
            ...
            if let Some(res) = codegraph_extract::extract_source(&path_str, &bytes) {
