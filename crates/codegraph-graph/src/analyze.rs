...
//! table avoids a `codegraph-detect` dependency. Bridge-node detection and the
...
/// Bundled analysis output, persisted as `.codegraph_analysis.json`.
...
/// `import_cycles` 20). Persisted as `.codegraph_analysis.json`.
pub fn analyze(
    kg: &KnowledgeGraph,
    communities: &BTreeMap<u32, Vec<NodeId>>,
    community_labels: &BTreeMap<u32, String>,
