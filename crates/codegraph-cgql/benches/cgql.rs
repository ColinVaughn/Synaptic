use codegraph_cgql::run;
use codegraph_core::{Confidence, Edge, FileType, GraphData, Node, NodeId, NodeKind, Span};
...
fn bench_cgql(c: &mut Criterion) {
    let kg = synthetic(2000);
    c.bench_function("cgql/property_filter", |b| {
        b.iter(|| {
            run(
                &kg,
    ...
    c.bench_function("cgql/relationship", |b| {
        b.iter(|| run(&kg, "MATCH (a:function)-[:calls]->(b) RETURN a, b"))
    });
}

criterion_group!(benches, bench_cgql);
