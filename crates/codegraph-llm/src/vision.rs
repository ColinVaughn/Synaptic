...
const OLLAMA_VISION_ENV: &str = "CODEGRAPH_OLLAMA_VISION";
        ...
        if read_bytes {
            match std::fs::read(p) {
                Ok(bytes) if bytes.len() > MAX_IMAGE_BYTES => {
                    eprintln!(
                        "[codegraph] image {rel} is {} KB, over the {} MB inline-image limit; \
                ...
                Err(e) => eprintln!("[codegraph] could not read image {rel}: {e}"),
            }
        }
