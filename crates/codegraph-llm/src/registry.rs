...
/// Resolve the `CODEGRAPH_LLM_TEMPERATURE` override. Returns:
...
fn temperature_override(get: &impl Fn(&str) -> Option<String>) -> Option<Option<f32>> {
    let raw = lookup(get, "CODEGRAPH_LLM_TEMPERATURE")?;
    let raw = raw.trim();
...
/// `CODEGRAPH_BACKEND` forces a specific backend (the only way to select the
...
pub fn resolve_backend(get: &impl Fn(&str) -> Option<String>) -> Option<&'static str> {
    if let Some(forced) = lookup(get, "CODEGRAPH_BACKEND") {
...
mod tests {
    ...
    mod unified {
        ...
        fn explicit_override_selects_opt_in_claude_cli() {
            ...
            assert_eq!(
                resolve_backend(&env(&[("CODEGRAPH_BACKEND", "claude-cli")])),
                Some("claude-cli")
            );
            // An unknown override name resolves to nothing.
            assert_eq!(
                resolve_backend(&env(&[("CODEGRAPH_BACKEND", "bogus")])),
                None
            );
        }
    ...
    mod wiring {
        ...
        async fn temperature_env_none_omits_the_parameter() {
            ...
            let get = env(&[
                ("OPENAI_API_KEY", "k"),
                ("OPENAI_BASE_URL", &server.uri()),
                ("CODEGRAPH_LLM_TEMPERATURE", "none"),
            ]);
            let client = make_backend(detect_backend(&get).unwrap(), &get).unwrap();
            client.complete("s", "u").await.unwrap();
