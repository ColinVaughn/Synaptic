...
impl KnowledgeGraph {
    ...
    pub fn from_graph_data(data: GraphData) -> KnowledgeGraph {
        ...
        for mut node in data.nodes {
            // `norm_label` is derived from `label` and regenerated by the export
            // path; nothing reads it back. Dropping it here lets a typical code
            // node hold an EMPTY `extra` map, and an empty BTreeMap allocates
            // nothing where one holding a single key costs a fixed ~632-byte
            // 11-slot leaf -- on a 379k-node graph that is the difference
            // between ~155 MB of side-table and none.
            if node.extra.remove("norm_label").is_some() && node.extra.is_empty() {
                // `BTreeMap::remove` does NOT release the node's allocation: a
                // map emptied by `remove` still holds its ~656-byte leaf, versus
                // 24 bytes for a freshly-constructed empty map. Measured.
                node.extra = serde_json::Map::new();
            }
            kg.upsert_node(node);
        }
...
mod tests {
    ...
    fn from_graph_data_drops_the_derived_norm_label() {
        let raw = serde_json::json!({
            ...
            "nodes": [{
                "id": "a", "label": "Auth.py", "file_type": "code",
                "source_file": "a.py", "norm_label": "auth.py", "_origin": "ast"
            }],
            "links": [], "hyperedges": []
        });
        ...
        assert_eq!(node.label, "Auth.py", "the source of truth is untouched");
    }
