...
//! and verify the graph after the agent applies it. CodeGraph never edits source.
//!
//! The pipeline reuses existing primitives: the loaded `KnowledgeGraph` for the
//! symbol set, `codegraph_query::affected_nodes` for the blast radius, the AST
//! cache (`codegraph_extract::cached_extract_source`) for column-accurate per-site
//! spans, `codegraph_incremental::rebuild` to re-extract for verify, and
//! `codegraph_graph::find_import_cycles` for the no-new-cycle invariant.
