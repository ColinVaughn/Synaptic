...
//!   overridable via `SYNAPTIC_TRANSCRIBE_CMD`) writes a `.txt`/`.vtt` we read.
//!
...
pub fn transcribe_media(media: &Path, target_dir: &Path) -> Result<PathBuf, FetchError> {
    std::fs::create_dir_all(target_dir).map_err(|e| FetchError::Http(e.to_string()))?;
    let cmd = std::env::var("SYNAPTIC_TRANSCRIBE_CMD").unwrap_or_else(|_| "whisper".into());
    let model = std::env::var("SYNAPTIC_WHISPER_MODEL").unwrap_or_else(|_| "base".into());
