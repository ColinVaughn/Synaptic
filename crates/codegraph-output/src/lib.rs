//! Output writers for CodeGraph. The canonical artifacts are `graph.json`
...
mod tests {
    ...
    use codegraph_graph::{apply_communities, cluster, ClusterOptions};
    use serde_json::Map;
    ...
    fn json_round_trips_to_disk() {
        ...
        let p = dir.path().join("codegraph-out/graph.json");
        to_json(&kg, &p).unwrap();
