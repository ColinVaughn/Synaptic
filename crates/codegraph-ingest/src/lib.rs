//! External-source ingestion for CodeGraph: URL/office/media ingestion, MCP
...
//! (image-vision lives in `codegraph-llm`).
#![forbid(unsafe_code)]
...
use codegraph_core::{sanitize_label, Confidence, Edge, FileType, Node, NodeId};
...
/// needn't depend on `codegraph-extract`).
