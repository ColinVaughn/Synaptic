...
//! (`crates/codegraph-skillgen/expected/`), resolved via `CARGO_MANIFEST_DIR`.
    ...
    for art in render_all() {
        ...
        match std::fs::read_to_string(&path) {
            ...
            Ok(_) => problems.push(format!(
                "expected/{} is out of date (run `codegraph skill bless`)",
                art.name
            )),
            Err(_) => problems.push(format!(
                "missing expected/{} (run `codegraph skill bless`)",
                art.name
            )),
        }
    }
...
mod tests {
    ...
    fn expected_tree_is_in_sync() {
        // CI guard: a template edit without `codegraph skill bless` fails here.
