//! Time-travel diff: build the Synaptic at a git revision and diff two revisions.
//!
//! Each revision is materialized into a throwaway `git worktree` and built with
//! the existing `synaptic_incremental::rebuild` pipeline (detect + extract +
//! resolve + cluster), so no extraction logic is duplicated here. Built graphs are
//! cached per commit SHA under `synaptic-out/history/`. The reporting layer
