...
mod tests {
    ...
    fn ctx(kg: &codegraph_graph::KnowledgeGraph) -> AuditCtx<'_> {
        AuditCtx { kg, root: None }
    }
    fn graph_from(j: serde_json::Value) -> codegraph_graph::KnowledgeGraph {
        codegraph_graph::KnowledgeGraph::from_graph_data(serde_json::from_value(j).unwrap())
    }
    ...
    fn query_graph(sql: &str) -> codegraph_graph::KnowledgeGraph {
        graph_from(json!({
            "nodes": [
                {"id":"app.f","label":"f()","file_type":"code","source_file":"a.py","kind":"function"},
                {"id":"sql:t","label":"t","file_type":"code","source_file":"s.sql","kind":"table"}
