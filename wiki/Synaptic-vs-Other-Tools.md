...
|---|---|---|---|---|---|---|
| **Synaptic** | Persistent code knowledge graph plus source-grounded repository memory that you query, diff across git history, forecast changes against, and refactor against, instead of re-reading source | Symbols + typed edges, clustered into communities; every edge tagged `Extracted` / `Inferred` / `Ambiguous`; nodes carry kind/visibility/line-spans; plus boundary edges across language / process / repo (HTTP, FFI, WebSocket, IPC, event bus), flagged dynamic-dispatch sites, and revision-aware memory anchors | tree-sitter, 30+ languages | Single static binary, local, offline by default | MCP server (32 core tools plus five memory-query tools) over the graph and temporal overlay, incl. content search, history/pitfalls/decisions, find-all-references, dynamic-dispatch hazards, change-contract recovery/verification, change forecasting, predictive test selection, edit-impact prediction, readiness audit, structural search, describe-node, time-travel diff, and plan-only rename | AGPL-3.0-or-later, open source |
...
server to operate, and its engine is open source under AGPL-3.0-or-later; the main
...
3.10+ and its libraries. [26] Synaptic's MCP server is larger (32 core tools plus
five repository-memory query tools versus graphify's 10 tools plus 6 resources)
and implements the 2025-11-25 protocol revision with
...
differently: current Synaptic versions use AGPL-3.0-or-later (network copyleft),
while graphify is MIT (permissive). [26]
