...
fn run_relocate(a: RelocateArgs) -> Result<()> {
    ...
    let out_dir = a
        .out
        .unwrap_or_else(|| PathBuf::from("synaptic-out/refactor"));
    let (json_path, md_path) = synaptic_refactor::emit::write_relocate_plan(&plan, &out_dir)
        .map_err(|e| anyhow!("{e}"))?;
    ...
    if a.json {
        ...
        println!(
            "Apply, then: synaptic refactor verify --plan {} --relocate",
            json_path.display()
        );
    }
    Ok(())
...
fn run_rename(a: RenameArgs) -> Result<()> {
    ...
    let (old, opt_id) = match (&id, kg.node(&synaptic_core::NodeId(name.clone()))) {
    ...
    let out_dir = out.unwrap_or_else(|| PathBuf::from("synaptic-out/refactor"));
    let (json_path, md_path) =
        synaptic_refactor::emit::write_plan(&plan, &out_dir).map_err(|e| anyhow!("{e}"))?;
...
fn print_rename_summary(plan: &RenamePlan, json_path: &Path, md_path: &Path) {
    ...
    println!("  guide: {}", md_path.display());
    println!(
        "Apply the edits, then: synaptic refactor verify --plan {}",
        json_path.display()
    );
}
