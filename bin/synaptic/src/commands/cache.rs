...
/// Remove `synaptic-out/cache` under `root` (and, with `recursive`, every such
/// directory beneath it). Only ever touches the synaptic-owned, regenerable
/// `synaptic-out/cache` subtree. Returns the directories actually removed.
pub(crate) fn clear_caches(root: &Path, recursive: bool) -> Result<Vec<PathBuf>> {
    let mut removed = Vec::new();
    let top = root.join("synaptic-out").join("cache");
    if top.is_dir() {
...
/// Bounded, noise-pruned walk collecting + removing nested `synaptic-out/cache`
/// dirs (skips `node_modules`/`.git`; does not descend into a matched `synaptic-out`).
pub(crate) fn find_member_caches(
    ...
    for entry in rd.filter_map(std::result::Result::ok) {
        ...
        if name == "synaptic-out" {
            ...
            continue; // don't descend into synaptic-out
        }
...
mod cache_clear_tests {
    ...
    fn make_cache(root: &std::path::Path) -> std::path::PathBuf {
        let dir = root.join("synaptic-out").join("cache").join("ast");
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("x.json"), b"{}").unwrap();
        root.join("synaptic-out").join("cache")
    }
