...
//! `codegraph serve` binary over stdio, and drive the protocol handshake plus a
...
fn mcp_stdio_conformance_over_the_real_binary() {
    ...
    assert_cmd::Command::cargo_bin("codegraph")
        .unwrap()
        .arg("extract")
        .arg(root)
        .assert()
    ...
    let mut child: Child = Command::new(env!("CARGO_BIN_EXE_codegraph"))
        .current_dir(root)
        .arg("serve")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
