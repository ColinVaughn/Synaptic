...
pub(crate) fn write_federated(
    build: &codegraph_workspace::workspace_build::WorkspaceBuild,
    root: &Path,
) -> Result<()> {
    ...
    for s in &build.surfaces {
        codegraph_workspace::export_surface::save_surface(
...
pub(crate) fn print_build_summary(build: &codegraph_workspace::workspace_build::WorkspaceBuild) {
    println!(
...
pub(crate) fn run_workspace(action: WorkspaceAction) -> Result<()> {
    use codegraph_workspace::workspace_build::{
    ...
    match action {
        WorkspaceAction::Init {
            ...
            use codegraph_workspace::manifest::{
        ...
        WorkspaceAction::Add { target } => {
            use codegraph_workspace::manifest::{load_manifest, write_manifest, RepoMember};
            let mut manifest = load_manifest(&root)
                .map_err(|e| anyhow::anyhow!("{e}"))?
                .unwrap_or_default();
        ...
        WorkspaceAction::Build { changed, directed } => {
            ...
            if changed {
                let outcome = codegraph_workspace::state::update_workspace(&root, &opts)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                ...
                write_federated(&build, &root)?;
                codegraph_workspace::state::record_state(&root, &build)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
                print_build_summary(&build);
            }
            Ok(())
        ...
        WorkspaceAction::Sync => {
            ...
            let outcome = codegraph_workspace::state::update_workspace(
                &root,
        ...
        WorkspaceAction::Status => {
            let (statuses, has_remote) = codegraph_workspace::state::workspace_status(&root)
                .map_err(|e| anyhow::anyhow!("{e}"))?;
            if statuses.is_empty() {
                println!("No members found (run `codegraph workspace init`?).");
            }
            for s in &statuses {
                println!(
                    "  {} — {}",
        ...
        WorkspaceAction::List => {
            ...
            let manifest_present = codegraph_workspace::manifest::load_manifest(&root)
                .map_err(|e| anyhow::anyhow!("{e}"))?
                .is_some();
        ...
        WorkspaceAction::Discover { path, depth, max } => {
            use codegraph_workspace::scan::{discover_sibling_repos, ScanOptions, SkipReason};
            use codegraph_workspace::workspace_build::federate_repos;
            ...
            let repos: Vec<codegraph_workspace::manifest::RepoMember> = res
                .repos
                .iter()
                .map(|c| codegraph_workspace::manifest::RepoMember {
                    name: c.name.clone(),
                    git: None,
                    rev: None,
                    subgraph: None,
